defmodule RunaWeb.Live.Project.IndexTest do
  ...
  describe "projects view" do
    feature "renders projects cards", ctx do
      for project <- ctx.projects do
        ctx.session
        |> visit("/projects")
        |> assert_has(Query.css("[aria-label='Project #{project.name} card']"))
      end
    end
    ...
    feature "renders edit button", ctx do
      for project <- ctx.projects do
        ctx.session
        |> visit("/projects")
        |> assert_has(Query.css("[aria-label='Edit #{project.name} project']"))
      end
    end
    ...
    feature "renders projects statistics", ctx do
      for project <- ctx.projects do
        ...
        ctx.session
        |> visit("/projects")
        |> find(Query.css("[aria-label='Project #{project.name} statistics']"))
        |> assert_has(
  ...
  describe "projects toolbar" do
    feature "renders create project button", ctx do
      ctx.session
      |> visit("/projects")
      |> assert_has(Query.css("[aria-label='Create new project']", text: "New"))
    end
  end
